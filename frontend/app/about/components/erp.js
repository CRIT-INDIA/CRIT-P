...
export default function ERPImplementationDiagram() {
  ...
  useEffect(() => {
    function handleResize() {
      ...
      // More responsive sizing with smoother transitions
      let s;
  ...
  useEffect(() => {
    ...
    // Debounced resize handler for smoother transitions
    let resizeTimer;
    const handleResize = () => {
  ...
  const getMobileCircleDimensions = () => {
    const baseRadius = isMobile ? 
      (size <= 250 ? 110 : size <= 300 ? 130 : 150) : 
      (size <= 350 ? 160 : 180);
    
    return {
      containerSize: baseRadius * 2 + 100, // Add padding for cards
      radius: baseRadius,
      cardSize: isMobile ? (size <= 250 ? 14 : 16) : 18
    };
  };
    ...
    <div className="erp-implementation-diagram transition-all duration-500 ease-in-out">
      ...
      <div className={`w-full min-h-[600px] flex flex-col items-center justify-center p-4 sm:p-6 lg:px-4 lg:py-8 transition-all duration-500 ${isMobile ? 'scale-95' : 'scale-105'}`}>
        {isMobile ? (
          ...
          <div className="w-full mx-auto space-y-8 transition-all duration-500 max-w-md">
              ...
              >
                ...
                {SEGMENTS.map((segment, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className={`absolute ${isMobile ? 'w-16 h-16' : 'w-20 h-20'} ${
                        colors[index]
